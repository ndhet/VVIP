...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/ndhet/izinvps/ipuk/ip | grep $MYIP | awk '{print $2}')
...
LLatest=`date`
Get_Data () {
git clone https://github.com/ndhet/backup.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    ...
    git config --global user.name "ndhet" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    git commit -m backup &> /dev/null
    git branch -M ipuk &> /dev/null
    git remote add origin https://github.com/ndhet/backup
    git push -f https://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@github.com/ndhet/backup.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/ndhet/backup/ipuk/$NameUser/$NameUser.zip"
sleep 1
